...
| | |
|---|---|
| **Repository** | [UiPathAgentHack2026MaestroMinds](https://github.com/ParasuramanAlakarsamy0407/UiPathAgentHack2026MaestroMinds) |
...
> [`OmniQuoteAI__Animated_ArchitectureFlow.html`](https://github.com/ParasuramanAlakarsamy0407/UiPathAgentHack2026MaestroMinds/blob/main/OmniQuoteAI__Animated_ArchitectureFlow.html) — open it in any browser and use the arrow keys / nav buttons to step through.
...
| `Batch.Assembly` | RPA | `Main.xaml` | Assembles carrier quote PDFs into a single batch for the Risk Agent |
...
| `OmniquoteAI` | Maestro (BPMN) | — | Single Maestro process orchestrating the full 10-stage pipeline end-to-end |
...
2. Open the project in **UiPath Studio**, connected to the same Orchestrator tenant used for this submission (UiPath Automation Cloud — Staging):
    ...
    ```
    https://staging.uipath.com/hackathon26_695/DefaultTenant
    ```

    > If judging from a different tenant, simply publish the included Maestro process to your own tenant — no Orchestrator Assets or Queues need to be pre-created, and no `project.json` / config values need to be edited, since all endpoints are fully hosted and self-contained.
...
3. Retrieve the Microsoft Edge download directory, then extract and return only the **parent directory** by removing the `Downloads` folder from the path.
    ...
    | Download Path | Return Value |
    |---|---|
    | `C:\Users\jhsjm\Downloads\` | `C:\Users\jhsjm\` |
    | `D:\Downloads\` | `D:\` |
    | `E:\MyFiles\Downloads\` | `E:\MyFiles\` |

    Pass the returned path into the Maestro BPMN flow argument named **`MachineDownloadPath`**.
...
Sample input files are already included in this repository under the **`/SampleInputs`** folder, ready to attach as-is:
...
Send an email (using the sample files from `/SampleInputs`, unmodified) to:
...
3. Review the AI Risk Agent's reasoning for each carrier quote:
   ...
   - **Flagged (High Risk / Exceptions):** review reasoning individually, then click **Accept (override)**.
...
├── OmniQuoteAI__Animated_ArchitectureFlow/   # Interactive architecture walkthrough
...
- **Faster Turnaround** — multi-day manual underwriting reduced to a ~8–10 minute automated pipeline
...
| **UiPath Data Fabric** | Real-time data storage |
| **UiPath Storage Bucket** | Stores all input and output files |
| **UiPath Integration Services** | Email integrations |
| **UiPath Apps** | Human-in-the-Loop UI |
| **Python + REST APIs** | Externally hosted carrier rating/quoting service (outside UiPath workflows) |
| **HTML + JavaScript** | Mock legacy carrier portal applications |
...
For questions about this submission, please reach out to any team member listed above or [open an issue on GitHub](https://github.com/ParasuramanAlakarsamy0407/UiPathAgentHack2026MaestroMinds/issues).
